export const CLIENT_ID = "<CLIENT ID GOES HERE>";
export const GATEKEEPER_AUTH_URL = "https://<SERVER URL GOES HERE>/authenticate/";
export const OAUTH_AUTHORIZE_URL = "https://github.com/login/oauth/authorize";
export const OAUTH_PROVIDER_NAME = "GitHub";
export const OAUTH_GH_REPO_STATUS_SCOPE = "repo:status";

export const GITHUB_ACCOUNT_NAME = "<GITHUB USERNAME WHOSE REPOS TO BE ACCESSED GOES HERE>";

export const GITHUB_ERR_BAD_CREDENTIALS = "Bad credentials";

export const GITHUB_REFS = {
  MASTER: "master"
};

export const GITHUB_STATUS_STATES = {
  PENDING: "pending",
  SUCCESS: "success",
  ERROR: "error",
  FAILURE: "failure"
};

export const HOME_PAGE_URI = "/";
